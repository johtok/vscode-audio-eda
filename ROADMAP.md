...
Status: `implemented` (drag + keyboard reordering, per-row transform selectors/settings, and persisted stack state are in place).
...
Status: `implemented` (second clip loading, side-by-side/stacked/overlay/difference rendering, time-offset alignment, and explicit trim start/duration controls).
...
Status: `implemented` (audio/speech/statistical/distributional metrics, histogram controls, and JSON/CSV export are available in the workbench).
...
Status: `implemented` (power/autocorrelation and short-time variants are computed and rendered from the timeseries view).
...
Status: `implemented` (PCA goal selector, explained-variance diagnostics, component selection, and classwise PCA views are available).
...
Status: `implemented` (channel-split rendering, multichannel toggles, and analysis-channel selection are in place).
...
Status: `implemented` (classwise metrics are computed from activation-derived classes with per-class diagnostics).
...
Status: `implemented` (JS symbolic transform, pattern importance explorer, prototype forest OOB diagnostics, plus interactive tree and pattern explorers).
...
Status: `implemented` (command flow + toolbox integration + in-workbench CASTOR visualizations with run presets and prototype vector diagnostics).
