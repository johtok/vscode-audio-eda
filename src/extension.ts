...
const MAX_COMPARISON_TRIM_SECONDS = 24 * 60 * 60;
...
const VALID_ANALYSIS_TOOLS = new Set(["rcluster", "random_forest", "castor", "spf"]);
...
function sanitizeWorkbenchStatePayload(rawState: unknown): unknown | undefined {
  ...
  if (comparison) {
    ...
    sanitized.comparison.trimStartSeconds = sanitizeNumber(
      comparison.trimStartSeconds,
      sanitized.comparison.trimStartSeconds,
      0,
      MAX_COMPARISON_TRIM_SECONDS
    );
    sanitized.comparison.trimDurationSeconds = sanitizeNumber(
      comparison.trimDurationSeconds,
      sanitized.comparison.trimDurationSeconds,
      0,
      MAX_COMPARISON_TRIM_SECONDS
    );
  }
  ...
  if (analysis) {
    ...
    ) as "rcluster" | "random_forest" | "castor" | "spf";
  }
