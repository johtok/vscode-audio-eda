    ...
    <main class="layout">
      <section class="panel stack-panel">
        ...
        <p id="stack-reorder-hint" class="hint">
          ...
          <strong>Arrow Up/Down</strong>, <strong>Home</strong>, <strong>End</strong>.
        </p>
        <p class="hint">Each row has its own <strong>Settings</strong> panel for transform-specific hyperparameters.</p>
        <p id="stack-a11y-status" class="sr-only" role="status" aria-live="polite"></p>
      ...
      <section class="panel controls-panel">
        ...
        <label class="row" for="comparison-trim-start-seconds">Second clip trim start (s)</label>
        <input id="comparison-trim-start-seconds" type="number" min="0" max="86400" step="0.01" />
        <label class="row" for="comparison-trim-duration-seconds">Second clip trim duration (s, 0 = full)</label>
        <input id="comparison-trim-duration-seconds" type="number" min="0" max="86400" step="0.01" />
        ...
        <select id="analysis-tool-select">
          ...
          <option value="castor">CASTOR Prototype</option>
        ...
        <section id="analysis-panel-castor" class="analysis-tool-panel">
          <h2>CASTOR Prototype (JS)</h2>
          <label class="row" for="castor-source">Feature source</label>
          <select id="castor-source">
            <option value="mel">Short-time mel features</option>
            <option value="stft">Short-time STFT spectrogram features</option>
          </select>
          <p id="castor-feature-hint" class="hint">
            Sequences are generated from short-time features of the current primary audio clip.
          </p>
          <p id="castor-label-hint" class="hint">
            Labels are generated from activation overlay intervals (active/inactive classes).
          </p>
          <label class="row" for="castor-preset">Run preset</label>
          <select id="castor-preset">
            <option value="balanced" selected>Balanced (default)</option>
            <option value="fast">Fast preview</option>
            <option value="deep">Deep diagnostics</option>
          </select>
          <label class="row" for="castor-max-frames">Max frames</label>
          <input id="castor-max-frames" type="number" min="128" max="8000" step="1" value="2200" />
          <label class="row" for="castor-pad-length">Pad length (0 = infer)</label>
          <input id="castor-pad-length" type="number" min="0" max="4096" step="1" value="0" />
          <label class="row" for="castor-top-dims">Top dimensions (why panel)</label>
          <input id="castor-top-dims" type="number" min="4" max="128" step="1" value="16" />
          <label class="row">
            <input id="castor-normalize" type="checkbox" checked />
            Normalize each sequence (z-score)
          </label>
          <button id="castor-run" type="button">Run CASTOR prototype</button>
          <progress id="castor-progress" class="castor-progress" max="100" value="0"></progress>
          <p id="castor-status" class="hint">Ready to run CASTOR prototype diagnostics.</p>
          <div id="castor-results" class="castor-results"></div>
        </section>
